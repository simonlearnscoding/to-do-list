...
export class projectComponents {
  ...
  static projectSelectCss(project) {
    ...
    getAllProjects.forEach((iteration) =>
      iteration.classList.remove("selected")
    );
...
export class taskComponents {
  static createProjects() {
    ...
    function createDefaultProjects(target) {
      const projects = [
        { name: "inbox", type: "project", kind: "default" },
        {
          name: "this week",
          type: "project",
          kind: "default",
        },
        { name: "this month", type: "project", kind: "default" },
      ];
  ...
  static loadTasks(project) {
    ...
    function filterByTimePeriod(project, tasks) {
      ...
      return tasks.filter((task) => {
        ...
        if (dateObj["daysDifference"] <= threshold) return true;
      });
