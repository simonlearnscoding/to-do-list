  ...
  static taskFormEdit(object) {
    ...
    const createPriorityPreselect = (object) => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        let line = `<option ${selected} value=\"${strings[i]}\">${
          i + 1
        }</option> \n`;
        str += line;
      }
    ...
    let options = projectComponents.getProjectOptions(taskproject);
    return `<input class="name" type="text" value="${
      object.name
    }" name="name" required />
           <input type="hidden"  name="type" value="tasks"/>
         ...
         <textarea class="desc" name="description" form="task-input-form"  cols="1" rows="1">${
           object.description
         }</textarea>
        <button class="cancel" id="cancel">Cancel</button>
